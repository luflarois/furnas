...
module sortsMod
 ...
 contains
   ...
   ! Note execution path independent of contents of array
   ! This version does not require array size to be power of 2
   subroutine odd_even_merge_sort(array)
        ...
        do while(k.ge.1)
           do j=mod(k,p),n-1-k,2*k
              do i=0,min(k-1,n-j-k-1)
                 if ((i+j)/(2*p).eq.(i+j+k)/(2*p)) then
                    tmp=max(array(i+j),array(i+j+k))
                    array(i+j)=min(array(i+j),array(i+j+k))
                    array(i+j+k)=tmp
                 end if
              end do
           end do
   ...
   ! See Knuth, The Art of Computer Programming, vol 3 (algorithm 5.2.2M)
   subroutine odd_even_merge_sort_knuth(array)
     real(prec), intent(inout) :: array(0:)
     integer :: i,p,p0,q,r,d,n
     real(prec) :: tmp
     ...
     do while(p.gt.0)
        ...
        do while(d.gt.0)
           do i=0,n-d-1
              if (iand(i,p).eq.r) then
                 tmp=max(array(i),array(i+d))
                 array(i+d)=min(array(i),array(i+d))
                 array(i+d)=tmp
              end if
           end do
           d=q-p
   ...
   end subroutine odd_even_merge_sort_knuth
